...
#  data: A numpy array representing the MNIST data ( x_train).
  # Reshape data to a 2D grid 
  data_reshaped = data.reshape(-1, 28 * 28)

  # Calculating the mean and standard deviation of the pixel intensities
  mean = np.mean(data_reshaped, axis=0)
  std = np.std(data_reshaped, axis=0)

  # Reshape mean and standard deviation back to a 28x28 grid
  mean_img = mean.reshape(28, 28)
  std_img = std.reshape(28, 28)

  # Create a subplot for mean and standard deviation heatmaps
  fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 5))

  # Plot mean heatmap
  ax1.imshow(mean_img, cmap='gray')
  ax1.set_title('Mean Pixel Intensity')
  ax1.set_xticks([])
  ax1.set_yticks([])

  # Plot standard deviation heatmap
  ax2.imshow(std_img, cmap='hot')
  ax2.set_title('Standard Deviation of Pixel Intensity')
  ax2.set_xticks([])
  ax2.set_yticks([])
...
# Example 
plot_distribution_heatmap(x_train)
